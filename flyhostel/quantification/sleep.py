...
def get_parser(ap=None):
    ...
    ap.add_argument("--ld-annotation", dest="ld_annotation", action="store_true", default=True)
    ap.add_argument("--no-ld-annotation", dest="ld_annotation", action="store_false", default=True)
    return ap
...
def sleep_annotation_all(data, **kwargs):
    ...
    if N_JOBS == 1:
        data_annot = []
        for i in tqdm(np.unique(data["id"]), desc="Quantifying sleep on animal"):
            data_annot.append(sleep_annotation(data.loc[data["id"] == i], **kwargs))
    else:
        data_annot = joblib.Parallel(n_jobs=N_JOBS)(joblib.delayed(sleep_annotation)(
...
def bin_apply(dt, analysis_params):
    ...
    dt_binned["L"]=["F" if e else "T" for e in (dt_binned["t"] / 3600) % 24 > 12]
...
def main(args=None, ap=None):
    ...
    data = tidy_dataset_all(velocities, chunk_metadata=(frame_number, frame_time), analysis_params=analysis_params)
    ...
    logger.info(f"Binning data every {analysis_params.summary_time_window/60} minutes")
