...
def get_parser(ap=None):
    ...
    ap.add_argument("--experiment-folder", "--input", dest="input")
    ap.add_argument("--reference_hour", "--zt0", dest="reference_hour")
    return ap


def read_data(store_path, reference_hour):
    """
...
def plot_data(store, data):
    ...
    experiment_date = os.path.basename(
            os.path.dirname(
                store.full_path
            )
        )
