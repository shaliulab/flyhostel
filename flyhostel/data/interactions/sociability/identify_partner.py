...
def draw_partner_fly_translucent(
    ...
    """
    Same as before, but returns (frame, status) where status=True if any part of
    the square outline was actually drawn into the crop.
    """
    import numpy as np

    H, W = frame.shape[:2]

    # Track whether we actually drew anything
    drawn = False
    ...
    def blend_set(ys, xs):
        ...
        if frame.ndim == 2:
            orig = frame[ys, xs].astype(np.float32) / maxv
            target = (float(blend_to) / maxv) if not isinstance(blend_to, (tuple, list)) else (float(blend_to[0]) / maxv)
            out = (1.0 - alpha) * orig + alpha * target
            frame[ys, xs] = np.clip(out * maxv, 0, maxv).astype(frame.dtype)
        else:
            orig = frame[ys, xs, :].astype(np.float32) / maxv
            if isinstance(blend_to, (tuple, list)):
                bt = np.asarray(blend_to, dtype=np.float32) / maxv
            else:
                bt = np.array([blend_to, blend_to, blend_to], dtype=np.float32) / maxv
            out = (1.0 - alpha) * orig + alpha * bt
