...
class H5Reader:
    ...
    def __init__(self, files, framerate, data_framerate=None, main_key="best"):
        """
        Args:
            fps (int):
        """
        ...
        if not self._files:
            ...
            with h5py.File(self._files[0], "r") as filehandle:
                ...
                try:
                    dataset=filehandle[self.main_key]
                except Exception as error:
                    print(f"Cannot open {self._files[0]}:{self.main_key}. Available: {filehandle.keys()}")
                    raise error

                self._class_names = tuple([name.decode() for name in dataset["class_names"][:]])
    ...
    def load(self, behavior, n_jobs, chunks=None, main_key=None):
        ...
        Output = joblib.Parallel(n_jobs=n_jobs)(
            ...
            )(file, behavior, self._class_names, step=self.step, main_key=main_key)
        ...
        if chunks is None:
            ...
            found_chunks = []
            for chunk_, probability in Output:
                if chunk_ in chunks:
                    p_list.append(probability)
                    n_frames+=len(probability)
                    found_chunks.append(chunk_)

            assert len(found_chunks) == len(chunks)

        hours = round(n_frames/self.data_framerate/3600, 2)
        logger.info(f"Loaded %f hours (%d chunks)", hours, len(chunks))
