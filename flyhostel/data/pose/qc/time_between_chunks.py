...
def get_time_between_chunks_from_dbfile(experiment):
    ...
    diff=df.loc[(df["frame_number"]+1)%chunksize<=1]["frame_time"].diff().iloc[::2]/1000
    diff=sorted(diff.values.tolist())
    return diff


def get_time_between_chunks_from_npz_files(experiment, sort=False):
    basedir=get_basedir(experiment)
    ...
    npz_files=sorted(glob.glob(f"{basedir}/*.npz"))
    records=[]
