...
from flyhostel.data.pose.ethogram.ethogram import postprocess_behaviors
...
def inference(
        ...
        frame_numbers=None, postprocess=True, deg_folder=None, **kwargs
    ):
    ...
    with open(model_path, "rb") as handle:
        model, _, _= pickle.load(handle)
   
    behaviors=model.classes_
    fps=loader.framerate/loader.wavelet_downsample
    # 1. RF inference
    predictions=inference_(
        dataset, "behavior",
        model_path=model_path, fps=fps,
...
def inference_(dataset, label, model_path, fps, deg_folder):
    ...
    if (missing_counts>0).any():
        ...
        dataset[features]=dataset[features].bfill(axis=0)
    ...
    preds=postprocess_behaviors(preds, deg_folder, percentile=1, column="prediction", behaviors=BEHAVIOR_IDX_MAP)
    ...
    preds=apply_rules(preds, fps=fps, micromovement_behavior="inactive+micromovement")
    ...
    if "behavior" in preds.columns:
        behavior_col="behavior"
    else:
        behavior_col=None
    preds=preds.groupby("id").apply(lambda df: annotate_bout_info(df, fps=fps, behavior=behavior_col, prediction="prediction2")).reset_index(drop=True)
    return preds


# def evaluate_model_on_test_set(test_set, model_path, output_folder, deg_folder, micromovement_behavior="inactive+micromovement", by_fly=False):
